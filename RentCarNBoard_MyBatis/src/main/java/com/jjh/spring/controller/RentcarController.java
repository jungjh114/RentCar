...
import com.jjh.spring.common.RentCarParsing;
...
public class RentcarController {
	...
	public String carReserveMain(HttpServletRequest request, Model model) {
		...
		List<RentCar> list = dao.selectAll().subList(num - 3, num);
	...
	public String carReserveResult(HttpServletRequest request, Model model) throws ParseException {
		RentReserveIDao dao = sqlSession.getMapper(RentReserveIDao.class);
		RentcarIDao rdao = sqlSession.getMapper(RentcarIDao.class);
		HttpSession session = request.getSession();
		...
		model.addAttribute("rentalFee", rentalFee);
